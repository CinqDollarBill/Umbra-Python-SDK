...
def _order(seq):
    return {"order_id": f"ord-m1-{seq}", "market_id": "m1", "user_id": "u", "side": "BUY_YES",
...
def test_get_orders_auto_paginates(make_client, server):
    ...
    def _orders(req: httpx.Request):
        ...
        if cursor == "":
            return httpx.Response(200, json={"user_id": "u", "next_cursor": "p2",
                                             "orders": [_order(1), _order(2)]})
        return httpx.Response(200, json={"user_id": "u", "next_cursor": "",
                                         "orders": [_order(3)]})
...
def test_get_orders_limit_slices(make_client, server):
    ...
    @server.route("GET", "/user/orders")
    def _orders(req: httpx.Request):
        return httpx.Response(200, json={"user_id": "u", "next_cursor": "more",
