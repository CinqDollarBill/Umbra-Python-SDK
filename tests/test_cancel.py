...
def _cancel_ok(req: httpx.Request) -> httpx.Response:
    ...
    return httpx.Response(200, json={
        "order_id": body["order_id"], "market_id": body["market_id"],
...
def test_cancel_rejected_raises(client, server):
    ...
    def _rej(req):
        return httpx.Response(200, json={
            "order_id": "ord-m1-7", "market_id": "m1", "status": "REJECTED",
...
def test_cancel_by_client_id(client, server):
    ...
    server.json_route("POST", "/orders", {
        "accepted": True, "reason": None,
        "validation": {"valid": True, "reason": None, "market_id": "m1",
                       "wallet_balance": "1", "required_collateral": "0",
                       "available_after_trade": "1"},
        "order": {"order_id": "ord-m1-9", "market_id": "m1", "user_id": "u",
                  "status": "OPEN", "reason": None, "fills": [],
    ...
    client.place_limit_order(market="m1", side="BUY_YES", price="0.5", size=10, client_order_id="abc")
...
def test_cancel_all(client, server):
    server.json_route("GET", "/user/orders", {
        "user_id": "u", "next_cursor": "", "orders": [
            {"order_id": "ord-m1-1", "market_id": "m1", "user_id": "u", "side": "BUY_YES",
             "book_side": "BID", "order_type": "LIMIT", "tif": "GTC", "limit_price": "0.5",
             "quantity": 10, "filled_quantity": 0, "remaining_quantity": 10, "status": "OPEN",
             "seq": 1, "ts": 1},
            {"order_id": "ord-m1-2", "market_id": "m1", "user_id": "u", "side": "SELL_YES",
             "book_side": "ASK", "order_type": "LIMIT", "tif": "GTC", "limit_price": "0.7",
             "quantity": 10, "filled_quantity": 0, "remaining_quantity": 10, "status": "OPEN",
             "seq": 2, "ts": 2},
        ],
    })
